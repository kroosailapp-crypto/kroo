...
import { IconUser, IconArrowLeft, IconSend, IconAnchor } from "@tabler/icons-react";
...
export default function BoatChat({ params }) {
  const { id } = use(params); // other user's id (crew member)
  const { user } = useAuth();
  ...
  useEffect(() => {
    supabase
      .from("crew_profiles")
      .select("name, avatar_url, positions")
      .eq("id", id)
      .single()
        ...
        {loading ? (
          ...
          <div className="flex-1 flex items-center justify-center">
            <p className="text-sm text-gray-400">
              Start the conversation with {otherProfile?.name || "this sailor"}.
            </p>
          </div>
        ) : (
          messages.map((msg) => {
